...
one. The sequence number is carried on every call and is what self-hosted gateway
reconstruction uses to prove a run complete; the dashboard does not read it and
groups solely by `x-rb-run`. `RBTRACE_DISABLE=1` turns the labelling off without
removing the import.
