...
Every ReasonBlocks setup sends agent LLM traffic through a **hosted capture gateway** and adds
the two-line **`rbtrace` shim**, which labels run boundaries on outbound requests. Nothing
changes inside your LLM SDK or agent framework.

This page walks through the gateway setup everyone uses, then the two shim paths, depending on
whether one process handles one job or many.
...
The `rbtrace` shim adds two HTTP headers (`X-RB-Run`, `X-RB-Seq`) on outbound LLM API calls at
the HTTP-transport level — `httpx` and `httpx2` (the Anthropic and OpenAI Python SDKs),
`aiohttp` (google-genai's async client) and `botocore` (boto3, which sends through `urllib3`
and so gets an event hook rather than a patched client). `install()` covers all four. Install
it once at process startup; wrap each job in a context manager when one process handles many
jobs.

<Note>
**Gateway only** is fine for capture and dashboard review, and is the right way to start
collecting on day one. It is not enough for training: with run labels 95% of runs are provably
minable, without them about 7%. Both are equally trustworthy — nothing is ever marked minable
when it is not — so what you lose is volume, not correctness.
</Note>
...
| --- | --- | --- |
| **A — Gateway only** | Env var | Capture, audit, dashboard review; starting collection before a deploy window |
...
Nothing beyond the env var (or the equivalent base-URL setting for your provider).
...
- Your agent is written in TypeScript or another non-Python stack: the shim is Python-only, so this is the path those agents are on.
...
| Agent that talks to two providers (say Anthropic and Fireworks) | Same gateway, one prefix each; B or C as above |
