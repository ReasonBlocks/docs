...
The helper supplies the source base URL and the `x-reasonblocks-key` header, and —
because importing it installs run labelling for the process — every model call made
through the client carries `x-rb-run` and `x-rb-seq`. That is the whole integration
for capture; the sequence numbers are what make a task's run provably complete.

Naming tasks yourself is optional and matters only when one process runs many jobs.
...
The helper uses explicit client settings; task headers are optional. Your application
...
generated helper performs `install()` itself when imported, so remove any manual
`rbtrace.client.install()` call you added earlier (a second call is a harmless
no-op); an old `shim.run()` / `shim.new_run()` scope can stay or become
`run_headers()` — both still name a task. Remove the old gateway base-URL setting
