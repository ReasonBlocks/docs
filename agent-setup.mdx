...
<Note>
**Availability:** the CLI and helper require `rbtrace==1.1.0`. Public PyPI
...
installation examples. The plain-text copy of this page at `/agent-setup.md` and
the hosted skill (`npx skills add https://docs.reasonblocks.com`) resolve once
this documentation release is published.
</Note>
...
through the client carries `x-rb-run` and `x-rb-seq`. The dashboard groups a
task's calls by `x-rb-run` and uses `x-rb-snapshot-id` for full-agent training.
The sequence number is carried on every call; self-hosted gateway reconstruction
uses it to prove a run complete. The dashboard does not read it — it groups solely
by `x-rb-run`.
...
The helper uses explicit client settings. Your application continues executing
its tools.
...
Once this documentation release is published (see the availability note above):
