...
The output lists up to `limit` importers and `limit` dependencies. When there are more, a `(+N more)` line is appended.
...
<ParamField path="limit" type="integer" default="15">
  Maximum number of importers and dependencies to list before truncating with a `(+N more)` line.
</ParamField>
