...
carried on every call; self-hosted gateway reconstruction uses it to prove a run
complete, while the dashboard records it without gating training on it. The labelling
patches only the HTTP transport the SDK uses, reads no bodies and sends nothing of its
own. `RBTRACE_DISABLE=1` turns it off; `python -m rbtrace doctor` reports whether calls
...
must name each task with `run_headers()`.
...
| `x-rb-seq` | Added automatically: 1, 2, 3 … within the task. Recorded by the dashboard; used by self-hosted gateway reconstruction, where a gap leaves the run unproven |
