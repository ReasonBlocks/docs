...
<ParamField path="unit_id" type="string">
...
<ParamField path="on_fraction" type="float" default="0.5">
  Probability of the `on` arm, in `[0, 1]`. Also passed to the report's SRM check as the expected split.
</ParamField>

The remaining keyword arguments mirror [`middleware()`](#middleware) (`run_id`, `agent_name`, `task`, `framework`, `model`, `codebase_id`, `org_id`, `project_id`, `metadata`). Experiment tags are stamped last, so a caller's `metadata` cannot override the arm.

### Returns

<ResponseField name="ReasonBlocksMiddleware" type="middleware">
  A run-scoped middleware, same type and lifecycle as [`middleware()`](#middleware). For the `off` arm it runs in passthrough mode — scoring and telemetry only, with the model request left untouched. The chosen arm is on `mw._run_metadata["arm"]`.
</ResponseField>
