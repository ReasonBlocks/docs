...
connecting your client and retaining a run ID across all calls in one task.
...
ReasonBlocks or wait for the release before running those commands.
</Note>

## 3. Verify capture

Run the local `doctor` check in your application environment and test the actual
entrypoint. When you run a real workflow, confirm that its records appear in
**Data**. The local check verifies configuration and capture-key presence; it
does not contact the service or validate credentials remotely.
