...
distillation pipeline, and trained model serving for your tenant.
...
| Trained model serving | Dashboard → go live after training report; your model primary, frontier backup |

Your agent process needs the gateway URL via `ANTHROPIC_BASE_URL`, your existing API key, and
the two-line client shim. No DuckDB files, no local `rbtrace proxy`, and no SSH access required
for the default path.

Anthropic Messages is the primary path for training and serving your trained model. OpenAI Chat
...
After local training completes, the same gateway can serve your trained model using
...
Self-hosted trained model serving after training:
...
**Agent surface (hosted or self-hosted):** gateway URL + the two-line shim + your existing agent code.
